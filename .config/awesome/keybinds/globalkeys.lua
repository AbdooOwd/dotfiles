...
local globalkeys = gears.table.join(
	...
	awful.key({ config.modkey }, "v", function()
	...
	end, { description = "runs rofi", group = "launcher" }),

	awful.key({ config.modkey }, "Tab", function()
		awful.spawn("rofi -show window")
	end, { description = "[rofi] displays active windows", group = "launcher" }),

	-- requires 'xscreensaver' + its extra data, and 'fortune' installed
	awful.key({ config.modkey, "Control" }, "p", function()
		awful.spawn("/usr/libexec/xscreensaver/phosphor --scale 3 --program fortune")
	end, { description = "xscreensaver's phosphor", group = "fun" }),

	-- open calculator
	awful.key({ config.modkey }, "c", function()
		awful.spawn("gnome-calculator")
	end, { description = "calculator", group = "misc" }),

	-- change keyboard layout
	awful.key({ config.modkey }, "#42"--[[g]], function() -- TODO: Should turn this into a utils.lua func
		local kbdcfg = config.kbd_cfg
		kbdcfg.current = kbdcfg.current % #kbdcfg.layouts + 1
		local new_layout = kbdcfg.layouts[kbdcfg.current]
		awful.spawn("setxkbmap -layout " .. new_layout .. "")
	end, { description = "switches keyboard layout", group = "misc" }),

	-- locks screen with xsecurelock
	awful.key({ config.modkey }, "Escape", function()
		awful.spawn.with_shell("~/.local/bin/my_xsecurelock")
	end, { description = "locks screen", group = "misc" }),
